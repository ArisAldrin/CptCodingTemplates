...
vector<PT> HalfPlaneIntersecion(vector<LN>& L){
    ...
    for(int i=0;i<(int)L.size();++i){
        ...
        while(q.size() > 1 && chk(q.back() , q[q.size() - 2] , L[i]))q.pop_back();
        while(q.size() > 1 && chk(q.front() , q[1] , L[i]))q.pop_front();
        q.push_back(L[i]);
    }
    while(q.size() > 1 && chk(q.back() , q[q.size() - 2] , q.front()))q.pop_back();
    while(q.size() > 1 && chk(q.front() , q[1] , q.back()))q.pop_front();
    if(q.size() < 3) return {};
    vector<PT> ans;
